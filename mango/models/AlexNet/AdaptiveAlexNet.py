...
class AdaptiveAlexNet(AlexNet, Registry):
    ...
    def __init__(self, n_classes, input_shape, *args, **kwargs):
        ...
        self.conv3a = DoubleConv2D(_n(384, 24), kernel_size=_k(3, 3), strides=1, activation=tf.nn.relu, padding='same')
        self.conv3b = Conv2D(_n(256, 16), kernel_size=_k(3, 3), strides=1, activation=tf.nn.relu, padding='same')
        self.dense1 = DoubleDense(_n(4096, 64), activation=tf.nn.relu)
