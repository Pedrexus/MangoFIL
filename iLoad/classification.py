...
def get_class_from_path(filepath, consider_collapse_level=True,
                        consider_antracnosis=True, max_class=3):
...
def get_fruit_number_from_path(filepath):
    name = os.path.basename(filepath)
    fruit_number = name.split('_')[0][1:]

    return int(fruit_number)


def get_fruit_side_from_path(filepath):
    name = os.path.basename(filepath)
    fruit_number = name.split('_')[1][1]
